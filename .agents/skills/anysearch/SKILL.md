---
name: anysearch
description: Use when the user explicitly asks to search with AnySearch, run AnySearch vertical or batch search, or extract a URL through the AnySearch API. Do not use for sensitive queries or without consent to send query data to api.anysearch.com.
license: Apache-2.0
---

## Overview

> Downstream packaging notice: the SKILL.md frontmatter was normalized for Codex compatibility and its trigger was limited to explicit AnySearch use. Runtime scripts are unchanged from upstream version 3.0.1.
